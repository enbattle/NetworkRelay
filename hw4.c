    ...
    if( FD_ISSET(udp_sock, &readfds) ) { 
      ...
      if ( udp_num_bytes == -1 ){
        ...
        if(strstr(buffer,"WHO") != NULL){
          ...
          udp_num_bytes = sendto(udp_sock, "ERROR SEND not supported over UDP\n", 34, 
          ...
          udp_num_bytes = sendto(udp_sock, "ERROR SHARE not supported over UDP\n", 35, 
            0, (struct sockaddr *) &client, fromlen);
        ...
        if(strstr(buffer,"LOGIN") != NULL){ //login command
          ...
          if(strstr(buffer,"LOGIN") != NULL){
            ...
            if(msglen < 1 || msglen > 990){
              ...
              printf("CHILD %lu: Sent ERROR (Invalid msglen)\n", pthread_self());
              continue;
            }
            send(fd, "OK!\n", 4, 0);
            ...
            int msglen = 5 + 1 + strlen(current_user.userid) + 1 + filelen_str_len + 1;
            char* msg = calloc(msglen+1, sizeof(char));
            strcat(msg, "SHARE ");
            strcat(msg, current_user.userid);
            strcat(msg, " ");
            strcat(msg, filelen_str);
            strcat(msg, "\n");
            send(msg_fd, msg, msglen, 0);
            ...
            for(int i = 0; i < num_chunks; i++){
              ...
              send(msg_fd, chunks[i], tcp_num_bytes, 0);
              free(chunks[i]);
            }
            free(msg);
            free(chunks);
...
User createUser(char* userid, int fd){
  active_users[num_active_users].fd = fd;
  active_users[num_active_users].userid = calloc(strlen(userid)+1, sizeof(char));
  strcpy(active_users[num_active_users].userid, userid);
  num_active_users++;
  return active_users[num_active_users-1];
}

void deleteUser(char* userid){
  for(int i = 0; i < num_active_users; i++){
    User u = active_users[i];
    if(strncmp(u.userid,userid, strlen(userid)) == 0){
      for(int j = i; j < num_active_users-1; j++){
        active_users[j].fd = active_users[j+1].fd;
        active_users[j].userid = realloc(active_users[j].userid, (strlen(active_users[j+1].userid)+1)*sizeof(char));
